...
- `2026-04-19` **Módulo 7 — endpoint propio `onplay_cart_update` en vez de `wc-ajax=update_cart`/`remove_from_cart`.** Un único handler con contrato `{cart_action, cart_item_key, qty?}` en `inc/cart-update.php`, nonce `onplay_cart`. Devuelve **los 3 fragments del tema** (`[data-onplay-cart-count]`, `[data-onplay-cart-body]`, `[data-onplay-cart-footer]`) + un payload plano `{item_count, subtotal, items: {key: {qty, subtotal}}}` usado para actualizar filas en `/carrito/` sin re-renderizar la lista. Razón: contrato consistente, mensajes de error localizados, y poder actualizar drawer y cart page con el mismo endpoint. Docblock en la cabecera del archivo documenta el contrato para mantenciones futuras. **Rollback:** los handlers wc-ajax nativos siguen disponibles; el cliente puede reemplazar `CartUpdate.send()` por llamadas a `wc-ajax=update_cart` (qty dict) y `wc-ajax=remove_from_cart`, aceptando perder el payload plano y tener dos contratos distintos.
- `2026-04-19` **Módulo 7 — stepper (−/input/+) siempre visible por ítem, no "dropdown al hover".** El drawer (`inc/cart-drawer.php`) y `/carrito/` (`woocommerce/cart/cart.php`) muestran la caja de qty como tres controles permanentes. Razón: mejor touch target en mobile (≥ 28×28 drawer / 32×32 cart), descubribilidad inmediata, y a11y sin JS adicional para revelarlo. El input acepta edición directa (change event → action=set). **Rollback:** ocultar los botones `+/−` y dejar sólo el input editable; el handler `set` sigue funcionando así que la lógica no cambia.
- `2026-04-19` **Módulo 7 — cupones soportados en MVP vía WC nativo, sin UI de "Deshacer" en drawer.** `wc_coupons_enabled()` condiciona el form de cupón en `/carrito/`. No se construyó snackbar de undo para remove en el drawer porque WC ya provee el banner nativo "se ha eliminado X · Deshacer" en `/carrito/` cuando el usuario borra ahí. En el drawer el gesto es intencional (botón `×` explícito), y agregar un undo flotante implicaba manejo de timers + estado que no rinde para el MVP. **Rollback:** implementar en `CartUpdate.send('remove', ...)` un toast con setTimeout + fetch a `onplay_cart_restore` (endpoint que recordaría el último ítem removido en sesión).
- `2026-04-19` **Módulo 7 — locale único `es_CL` en MVP; multi-idioma (UK/BR) diferido a post-MVP.** Los textos del drawer y `/carrito/` están traducibles con text domain `onplay` pero no hay switcher ni archivos `.mo`. El inventario existente (`pa_idioma` = EN/ES/JA/ZH/PT/IT) se refiere al idioma *impreso de la carta*, no a la UI. **Rollback / ampliación:** agregar Polylang o WPML, crear `.mo` es_CL / en_US / pt_BR, y switcher en header con banderas; el tema ya usa `__()` en todos los strings nuevos del M7 así que no requiere edit de templates.
- `2026-04-19` **Módulo 8 — campo RUT chileno como `_billing_rut` con validación módulo 11 (PHP + JS espejo).** `inc/checkout-rut.php` inyecta `billing_rut` en `woocommerce_checkout_fields` (prioridad 25), valida en `woocommerce_after_checkout_validation` y guarda con `woocommerce_checkout_create_order`. El JS (módulo `CheckoutRut` en `main.js`) replica la validación y formatea a `12.345.678-K` en blur para UX inmediata; PHP es la fuente de verdad. **Decisión de alcance MVP:** RUT aparece en el metabox admin del pedido (`woocommerce_admin_order_data_after_billing_address`) y en el email interno al admin (`woocommerce_email_order_meta_fields` solo con `$sent_to_admin=true`) — **no en el email al cliente**, que queda diferido a Fase 2 cuando definamos documentación tributaria SII. **Rollback:** eliminar el require en `functions.php` + el include. La meta `_billing_rut` queda en las órdenes existentes, no es destructivo.
- `2026-04-19` **Módulo 8 — checkout un paso con layout 2-col y secciones numeradas (01 Contacto · 02 Despacho · 03 Notas · 04 Resumen/pago).** `woocommerce/checkout/form-checkout.php` se re-escribe desde cero pero delegando los campos via `woocommerce_form_field()` y los action hooks de WC (`woocommerce_checkout_order_review`, `woocommerce_review_order_before_payment`, etc.) para no romper gateways ni plugins. `review-order.php` override con miniatura+qty badge+variante+subtotal por item. Razón: fidelidad al diseño (hero del CLAUDE.md §5.5) + control total de skin sin luchar con CSS de plugin. **Rollback:** borrar `woocommerce/checkout/` y WC vuelve a sus plantillas default con sus clases originales.
- `2026-04-19` **Módulo 8 — campos WC default quitados para MVP: `billing_company`, `billing_state`, y ship-to-billing por defecto (`woocommerce_ship_to_different_address_checked` → false).** Razón: reducir fricción (los casos con company y ship distinto son minoría en Chile para singles). La checkbox "enviar a dirección distinta" SÍ se muestra — solo arranca unchecked. **Rollback:** borrar los `unset()` en `onplay_checkout_fields()` y el filtro `__return_false` para recuperar los campos y el default-checked original.
- `2026-04-19` **Módulo 8 — guest checkout habilitado en MVP, sin forzar registro.** Decisión del dueño basada en comportamiento del sitio actual (pocos usuarios se registran). La checkbox "Crear cuenta" aparece si WC la tiene habilitada (`$checkout->is_registration_enabled()` + `!is_registration_required()`). **Rollback:** en WC admin, activar "Require account creation during checkout" → el form renderea campos de cuenta obligatorios sin tocar el template.
- `2026-04-19` **Módulo 8 — shipping zones + payment gateways + sandbox keys DIFERIDOS al dueño.** M8 quedó con el código completo (RUT + form + review + thankyou + SCSS) pero el **criterio de aceptación "pedido test end-to-end en sandbox + email + orden en admin con todos los campos" NO PUEDE validarse hasta que el dueño configure en WC admin:** (a) Shipping zones (Chile > Retiro en tienda `free` + Chilexpress `flat rate`), (b) Webpay Plus con keys sandbox Transbank, (c) Mercado Pago con keys sandbox developer.mercadopago.cl. Sin estos el checkout no tiene métodos ni gateways y el submit falla. **Post-config:** hacer un pedido test con Webpay test cards + MP sandbox, verificar que el RUT aparece en admin y email interno, y recién ahí cerrar M8 formalmente con un commit `feat(M8-validation)` o similar.
- `2026-04-19` **Módulo 8 — páginas WC con shortcode clásico, NO bloques.** Tras activar el tema, las páginas "Carrito" y "Finalizar compra" tenían el bloque Gutenberg `wp:woocommerce/checkout` (default de WC 10) y nuestros overrides PHP (`woocommerce/cart/cart.php`, `woocommerce/checkout/form-checkout.php`) no se ejecutaban — HTML generado con clases `wc-block-components-*`, labels en inglés y botón "Place Order" gris. **Fix:** editar ambas páginas en wp-admin (editor de código), borrar TODO el contenido con `<!-- wp:... -->` y pegar respectivamente `[woocommerce_cart]` y `[woocommerce_checkout]`. Paso crítico para que los overrides del tema funcionen. Documentado aquí para que el checklist de activación del tema (Módulo 10) incluya esta verificación automática o, idealmente, un `wp post update` en el onboarding del tema.
- `2026-04-19` **Módulo 8 — remove_action() de `woocommerce_before_checkout_form` para cupón y login.** El toggle "¿Tienes un cupón?" y el "Returning customer? Click here to login" se removieron del header del checkout: el cupón ya está en /carrito/ (decisión M7) y con guest checkout habilitado el login en el flujo de compra es fricción. El hook sigue disparándose porque otros gateways/plugins se enganchan ahí. **Rollback:** borrar los dos `remove_action()` en `inc/checkout-rut.php`.
- `2026-04-19` **Fix `/product-category/<slug>/` → redirect 302 a `/tienda/?set=<slug>`.** Nuestro `archive-product.php` construye el estado de filtros leyendo solo `$_GET`, así que al aterrizar en el archive nativo de `product_cat` (breadcrumbs, enlaces externos, Google Shopping futuro) la categoría se perdía y se mostraba el catálogo completo. Con el redirect hay **una sola URL shareable** para el listado filtrado y el chip del set queda visible. 302 (no 301) para permitir revisar estrategia en M10 (SEO) — si queremos canonical por category page, cambiamos el redirect por inyectar la cat en el state. Implementado en `inc/woocommerce.php` vía `template_redirect`.
- `2026-04-19` **Fix drawer — footer/totals/ctas/hint estaban mal-anidados en SCSS.** En `_cart-drawer.scss` los bloques `&__footer`, `&__totals`, `&__subtotal-row`, `&__subtotal-amount`, `&__hint` y `&__ctas` quedaban dentro de `.cart-row__error` en vez de `.cart-drawer`, así que compilaban como `.cart-row__error__footer` (selector que no matcheaba el DOM) y el footer aparecía completamente sin estilos: textos y botones pegados a la izquierda, sin grid 2-col, subtotal sin espaciado. **Corrección:** mover los 6 bloques de vuelta dentro de `.cart-drawer { ... }`.
- `2026-04-19` **Módulo 9 — Home cableada en PHP fijo con template-parts aislados.** `front-page.php` llama 4 `get_template_part()` de `template-parts/home/*` (hero, trust-band, featured-sets, recent-cards). **Decisión explícita:** NO bloques Gutenberg custom en MVP. Cada template-part es independiente y puede convertirse en render callback de un bloque en un módulo posterior sin mover lógica. **Rollback / migración a bloques:** crear `inc/blocks.php` + `blocks/hero-search/block.json` con `render.php` que include el template-part existente. Los selectores CSS no cambian.
- `2026-04-19` **Módulo 9 — featured sets ordenados por fecha del producto más reciente, no por volumen.** Query SQL directa en `onplay_home_get_featured_sets()`: `GROUP BY term_id ORDER BY MAX(post_date) DESC LIMIT 6`. Criterio del dueño: dar prioridad a los sets recién ingresados al catálogo (más relevante que los sets más grandes). Cacheado con transient 1h, invalidado en `save_post_product`. **Rollback:** cambiar `ORDER BY latest` por `ORDER BY tt.count` dentro de la query en `inc/home.php`.
- `2026-04-19` **Módulo 9 — hero con form simple `?q=` submit a /tienda/, sin autocomplete.** El módulo `Search` de `main.js` (M5) se engancha con `querySelector('[data-onplay-search]')` — un solo listener. Duplicarlo en hero requería refactor. MVP: hero submite `?q=` y cae al listado filtrado por búsqueda (ya soportado en `filters-ajax.php:201-203`). Usuario puede usar autocomplete desde el input del header que sigue siempre visible. **Rollback:** cambiar `querySelector` → `querySelectorAll` en `Search.init()` y loopear, o crear módulo `HeroSearch` espejo.
- `2026-04-19` **Módulo 9 — trust band con 3 pills y brands como chips de texto (no imágenes).** Retiro · Despacho · Pago seguro. El pill de pago enumera Webpay, Mercado Pago, Visa, Mastercard, Amex como `<span>` con tipografía mono sobre fondo oscuro — evita payload de assets, copyright issues de logos y mantiene el look minimalista. **Rollback / mejora:** reemplazar los `<span>` por `<img>` a SVGs oficiales cuando el dueño provea assets de marca.
- `2026-04-19` **Set icons: Keyrune webfont (no Scryfall API).** `onplay_render_set_icon($code, $size)` renderiza `<i class="ss ss-{code-lower}">` desde el CSS de Keyrune (cdn.jsdelivr). Razón: 1 font load (~40KB) cubre ~500 sets históricos + recientes, MIT, se tiñe con CSS (hereda `color`), sin fetches per-set en runtime. Scryfall `icon_svg_uri` requería N requests + caché server-side + invalidación — sobreingeniería. Nuevo helper `onplay_render_set_badge($label, $size)` conserva el diamante monograma para call sites que sólo tienen nombre/slug de categoría (filters-sidebar, home/featured-sets) — el código del set sólo vive en el SKU y todavía no se cachea como term_meta. **Rollback / migración:** reemplazar el `<i class="ss ss-...">` por SVG inline de Scryfall en el helper + caché server-side (`_onplay_set_icon_svg` en term_meta) si queremos self-host completo. Para cubrir Keyrune en filters/featured-sets, agregar un backfill que guarde `_onplay_set_code` como term_meta leyendo un producto sample.
...
- `2026-04-19` **Módulo 9 — Imagen PDP con srcset responsivo + columna ajustada.** `woocommerce/content-single-product.php` ahora usa `get_the_post_thumbnail($pid, 'full', ['sizes' => '(max-width: 640px) 100vw, (max-width: 960px) 60vw, 500px'])`. WP auto-genera `srcset` a partir de las registered sizes del attachment; el browser elige la mejor resolución según viewport + DPR (paridad con el slide 493px + srcset 200/400/600/800/1000 de TCGPlayer). Grid PDP cambia de `1fr 1.15fr` a `minmax(0, 500px) minmax(0, 1fr)` — la imagen queda contenida en 500px max (tamaño óptimo para cartas Magic con aspect 5/7 ≈ 357×500) y los detalles se expanden. Se cambió `object-fit: cover` → `contain` para no recortar art en cartas con aspect no-estándar (tokens, art cards). **Rollback:** volver a `'large'` + `object-fit: cover` + grid `1fr 1.15fr`. Nota: si el dueño no tiene imágenes grandes en el Media Library (el manager descarga de Scryfall en `normal` 488px), `'full'` devuelve esa resolución tal cual y el srcset será limitado — para srcset más rico, re-descargar desde Scryfall en `large` 672px o `png` 745px vía un comando de re-sync.
